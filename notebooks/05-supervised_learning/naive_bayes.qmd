...
1. We keep symbols and numbers. 
  Typically, we'd remove them. 
...
3. To further reduce sparsity, we replace numbers in texts with a placeholder `NUM` (as far as possible)
4. We create $n$-grams (unigrams, bigrams, trigrams) to capture multi-word expressions that might be informative (e.g., "stock market", "interest rate").
5. We lemmatize the tokens to reduce sparsity (e.g., "running", "ran", "runs" &rArr; "run").
6. We remove stopwords to focus on content words.
7. We remove punctuation to reduce noise.
...
toks <- df |> 
  ...
  tokens(remove_symbols=FALSE, remove_numbers=FALSE, xptr = TRUE) |> 
  tokens_tolower()
...
nb_model <- textmodel_nb(
  ...
  distribution = "Bernoulli" # set this when there are only 2 label classes ("multinomial" otherwise)
)
```
...
# make sure that all features for which there are a parameters are  in the DFM
newdata <- quanteda.textmodels:::force_conformance(newdata, colnames(nb_model$param), force = TRUE)

# binarize feature indicators (aka one-hot-encoding)
newdata <- dfm_weight(newdata, "boolean", force = TRUE)

# get log of parameters 
present <- log(nb_model$param)
nonpresent <- log(1 - nb_model$param)
dim(present); dim(nonpresent) 

# get the theshold
threshold <- .Machine$double.eps
# clip infinte values
present[is.infinite(present)] <- max(-1e+05, log(threshold))
nonpresent[is.infinite(nonpresent)] <-max(-1e+05, log(threshold))

# compute probability of positive class
presence_prob <- Matrix::tcrossprod(newdata, present)

# compute probability of negative class 
nonpresence_prob <- matrix(
  base::colSums(t(nonpresent)),
  nrow = nrow(presence_prob),
  ncol = ncol(presence_prob),
  byrow = TRUE
) - Matrix::tcrossprod(newdata, nonpresent)

# get the log likelihood of examples for each class
loglik <- presence_prob + nonpresence_prob


# get the class-conditional log likelihood
logpos <- t(t(loglik) + log(nb_model$priors))

# convert to proability
# NOTE: init empty matrix
result <- matrix(
  NA,
  ncol = ncol(logpos),
  nrow = nrow(logpos),
